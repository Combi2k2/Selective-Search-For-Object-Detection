...
static image<float> *smooth(image<float> *src, float sigma) {
    vector<float> mask = make_fgauss(sigma);
    normalize(mask);
...
/* compute laplacian */
static image<float> *laplacian(image<float> *src) {   //this filter is used for highlighting the outline of objects, else it will nearly be completely canceled
    int width  = src -> width();
